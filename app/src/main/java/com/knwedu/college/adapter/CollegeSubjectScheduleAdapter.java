...
public class CollegeSubjectScheduleAdapter extends BaseAdapter {
	ViewHolder holder;
	...
	public CollegeSubjectScheduleAdapter(Context context,
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		if (convertView == null) {
			convertView = inflater.inflate(R.layout.college_subject_schedule_items,
					null);
		...
		CollegeAppUtils.setFont(context, holder.name);
		CollegeAppUtils.setFont(context, holder.marks);
			...
			public void onClick(View v) {
				...
				for (int i = daily.size() / weekly.size() * position; i < daily
					...
					viewHolder.date.setText("Date: "
							+ CollegeAppUtils.getDateFormat(daily.get(i).date));
