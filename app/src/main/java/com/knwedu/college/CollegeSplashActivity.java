...
public class CollegeSplashActivity extends ActionBarActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mContext = CollegeSplashActivity.this;

        //Remove Badge
        ShortcutBadger.removeCount(this);

        page_title = CollegeSplashActivity.this.getTitle().toString();
        Toast.makeText(mContext, "Title: " + page_title, Toast.LENGTH_SHORT);
        ...
        // No internet
        if (!CollegeAppUtils.isOnline(mContext)) {
            // Check already Logged in or not

            if (!CollegeAppUtils.GetSharedParameter(getApplicationContext(), CollegeConstants.USERTYPEID).equals("0")) {
                // Logged in
            ...
            // No internet and first time login
                ...
                CollegeAppUtils.showDialog(mContext, "Alert", "No internet Connectivity");
            }
        ...
        // Internet Available
            ...
            // For first time app installation
                ...
                // Go to Sign in page
                startActivity(new Intent(mContext, CollegeSigninActivity.class));
                finish();
                ...
                if (json != null) {
                    if (json.getString("result").equalsIgnoreCase("1")) {
                        ...
                        if (menu_tag != null) {
                            ...
                            CollegeAppUtils.SetSharedParameter(mContext, CollegeConstants.MENU_TAGS, menu_tag.toString());
                            CollegeAppUtils.SetSharedParameter(mContext, CollegeConstants.MENU_TITLES, menu_title.toString());
                        }
        ...
        protected void onPostExecute(Boolean result) {
            ...
            if (result) {
                if (curVersion.equalsIgnoreCase(version.version_name)) {
                    ...
                    if (CollegeAppUtils.GetSharedBoolParameter(mContext, CollegeConstants.ISSIGNIN) || (CollegeAppUtils.GetSharedBoolParameter(mContext, CollegeConstants.UISSIGNIN))) {
                        if (CollegeAppUtils.GetSharedBoolParameter(mContext, CollegeConstants.ISPARENTSIGNIN)) {
                            startActivity(new Intent(mContext, CollegeParentMainActivity.class));
                            finish();
                            return;
                        }
                        if (CollegeAppUtils.GetSharedBoolParameter(mContext, CollegeConstants.UISSIGNIN)) {
                            startActivity(new Intent(mContext, CollegeMainActivity.class));
                            finish();
                            return;
                        }
                        if (CollegeAppUtils.GetSharedParameter(mContext, CollegeConstants.USERTYPEID).equalsIgnoreCase("4")) {
                            startActivity(new Intent(mContext, CollegeMainActivity.class));
                            finish();
                            return;
                        } else if (CollegeAppUtils.GetSharedParameter(mContext, CollegeConstants.USERTYPEID).equalsIgnoreCase("3")) {
                            // Download data for offline access
                            new GetOfflineDataAsyntask().execute();
                            return;
                        } else if (CollegeAppUtils.GetSharedParameter(mContext, CollegeConstants.USERTYPEID).equalsIgnoreCase("5")) {
                            startActivity(new Intent(mContext, CollegeParentMainActivity.class));
                            finish();
                            return;
                        }
                ...
                if (error != null) {
                    if (error.length() > 0) {
                        CollegeAppUtils.showDialog(CollegeSplashActivity.this, page_title, error);
                    } else {
                        CollegeAppUtils.showDialog(mContext, page_title,getResources().getString(R.string.please_check_internet_connection));
                    }
                } else {
                    CollegeAppUtils.showDialog(mContext, page_title, getResources().getString(R.string.please_check_internet_connection));
                }
    ...
    private class GetOfflineDataAsyntask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            nameValuePairs.add(new BasicNameValuePair("user_type_id", CollegeAppUtils.GetSharedParameter(mContext, CollegeConstants.USERTYPEID)));
            nameValuePairs.add(new BasicNameValuePair("organization_id", CollegeAppUtils.GetSharedParameter(mContext, CollegeConstants.UORGANIZATIONID)));
            nameValuePairs.add(new BasicNameValuePair("user_id", CollegeAppUtils.GetSharedParameter(mContext, CollegeConstants.USERID)));
            // Log parameters:
            ...
            CollegeJsonParser jParser = new CollegeJsonParser();
            JSONObject json = jParser.getJSONFromUrlnew(nameValuePairs, CollegeUrls.api_activity_get_subjects);
            try {
    ...
    private class GetOfflineDataStudentAsyntask extends AsyncTask<List<NameValuePair>, Void, Boolean> {
        ...
        protected Boolean doInBackground(List<NameValuePair>... params) {
            ...
            CollegeJsonParser jParser = new CollegeJsonParser();
            JSONObject json = jParser.getJSONFromUrlnew(nameValuePairs, CollegeUrls.api_students_info);
            try {
