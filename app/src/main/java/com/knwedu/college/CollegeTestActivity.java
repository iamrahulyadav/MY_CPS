...
public class CollegeTestActivity extends Activity {
	private TextView title, assignmentText, duaDate, header, marks, comments,comment_main_txt;
	...
	private void initialize()
	{
		...
		assignmentText = (TextView) findViewById(R.id.assignment_txt);
		duaDate = (TextView) findViewById(R.id.assignment_due_txt);
		...
		{
			assignment_type = getIntent().getIntExtra(CollegeConstants.ASSIGNMENT_TYPE, 2);
			String temp = getIntent().getExtras().getString(CollegeConstants.TEST);
		...
		if(assignment != null)
		{
			...
			assignmentText.setText(assignment.description);
			duaDate.setText(assignment.created_date);
			...
			if(!assignment.marks.equalsIgnoreCase("null") && assignment.is_published.equals("1"))
		    {
			...
			id = CollegeAppUtils.GetSharedParameter(getApplicationContext(),
					"id");
			assign = "Assignment";
			((TextView)findViewById(R.id.assigned_date_txt)).setText("" +assignment.submit_date);
			if(assignment.file_name != null)
			{
				if(assignment.attachment.length()>0)
				{
			...
			if(assignment.is_published.equals("1")){
				...
				graphBtn.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View arg0) {
						...
						Intent intent = new Intent(CollegeTestActivity.this, CollegeGraphViewActivity.class);
						intent.putExtra("subject_id", assignment.subject_id);
						intent.putExtra("object_id", assignment.id);
						intent.putExtra(CollegeConstants.ASSIGNMENT_TYPE, "Test");
	...
	private void showDialog() {
		...
		txtView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				i.putExtra("Download_Test", Url);
				i.putExtra("from", "Test");
				startActivity(i); */
				CollegeAppUtils.imagePdfViewDocument(CollegeTestActivity.this,Url,assignment.file_name);
				dialog2.dismiss();
			}
		});
