...
public class ActivityFees extends AppCompatActivity {
    ...
    private TextView student_name, class_name, section_name, lbl_commission,
            commission, lbl_convenience, convenience, lbl_total_amnt, total_amnt,
            btn_pay, btn_view_image, emptyView, btnPaymentHistory;
    ...
    float comm_amount;
    private ArrayList<DataStructureFramwork.PaymentHistory> paymentHistory = new ArrayList<>();
    private ListView listviewPaymentHistory;
    private PaymentHistoryDetailsAdapter adapter;
    private RelativeLayout bodylayout;
    private LinearLayout feelayout, header_layout;
   // private ImageButton btnBack;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.aims_manage_fee_view);
    ...
    private void initialize() {
        JSONObject c = null;
        context = ActivityFees.this;
        student_name = (TextView) findViewById(R.id.stduent_name);
        class_name = (TextView) findViewById(R.id.class_name);
        section_name = (TextView) findViewById(R.id.section_name);
        lbl_commission = (TextView) findViewById(R.id.lbl_commission);
        commission = (TextView) findViewById(R.id.commission);
        lbl_convenience = (TextView) findViewById(R.id.lbl_convenience);
        convenience = (TextView) findViewById(R.id.convenience);
        lbl_total_amnt = (TextView) findViewById(R.id.lbl_total_amnt);
        total_amnt = (TextView) findViewById(R.id.total_amnt);
        amount = (EditText) findViewById(R.id.amount);
        reg_no = (EditText) findViewById(R.id.reg_number);
        aims_fee_type = (EditText) findViewById(R.id.aims_fee_type);
        btn_pay = (TextView) findViewById(R.id.btn_pay);
        btn_view_image = (TextView) findViewById(R.id.btn_view_image);
        List<NameValuePair> nameValuePairs = new ArrayList<NameValuePair>(
        );
        nameValuePairs.add(new BasicNameValuePair("organization_id", SchoolAppUtils.GetSharedParameter(context, Constants.UORGANIZATIONID)));
        new GetCommission().execute(nameValuePairs);


        try {
            c = new JSONObject(SchoolAppUtils.GetSharedParameter(context, Constants.SELECTED_CHILD_OBJECT));
        ...
        btn_pay.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (errorCount == 0) {
                    ...
                    if (SchoolAppUtils.isOnline(context) && pg_convenience_fee != null && pg_commission != null) {

                        List<NameValuePair> nvp = new ArrayList<NameValuePair>();
                        nvp.add(new BasicNameValuePair("parent_id", SchoolAppUtils.GetSharedParameter(context, Constants.USERID)));
                        nvp.add(new BasicNameValuePair("student_id", SchoolAppUtils.GetSharedParameter(context, Constants.CHILD_ID)));
                        nvp.add(new BasicNameValuePair("pg_master_id", "3"));
                        nvp.add(new BasicNameValuePair("registration_num", reg_no.getText().toString()));
                        nvp.add(new BasicNameValuePair("amount", Float.toString(total_amount)));
                        nvp.add(new BasicNameValuePair("fee_type", aims_fee_type.getText().toString()));
                        nvp.add(new BasicNameValuePair("orig_amount", amount.getText().toString()));
                        nvp.add(new BasicNameValuePair("organization_id", SchoolAppUtils.GetSharedParameter(context, Constants.UORGANIZATIONID)));
                        new GetCCavenueInformation().execute(nvp);
                    } else {
                        SchoolAppUtils.showDialog(context, "Alert", "Connectivity problem. Please try again.");
        ...
        nvpair.add(new BasicNameValuePair("user_id", SchoolAppUtils
                .GetSharedParameter(context,
                        Constants.CHILD_ID)));
        nvpair.add(new BasicNameValuePair("organization_id", SchoolAppUtils.GetSharedParameter(context,
                Constants.UORGANIZATIONID)));
        new GetRegistrationNumber().execute(nvpair);
        //----------------view history------------------//
        listviewPaymentHistory = (ListView) findViewById(R.id.listviewPaymentHistory);
        emptyView = (TextView) findViewById(R.id.emptyView);
        btnPaymentHistory = (TextView) findViewById(R.id.btnPaymentHistory);
        bodylayout = (RelativeLayout) findViewById(R.id.bodylayout);
        header_layout = (LinearLayout) findViewById(R.id.header_layout);
        feelayout = (LinearLayout) findViewById(R.id.feelayout);
    ...
    private void requetsForPaymentHistory() {
        ...
        nameValuePairs.add(new BasicNameValuePair("organization_id", SchoolAppUtils.GetSharedParameter(context, Constants.UORGANIZATIONID)));
        nameValuePairs.add(new BasicNameValuePair("user_id", SchoolAppUtils.GetSharedParameter(context, Constants.USERID)));
                ...
                if (json != null) {
                    if (json.getString("result").equalsIgnoreCase("1")) {
                        ...
                        for (int i = 0; i < array.length(); i++) {
                            DataStructureFramwork.PaymentHistory payment = new DataStructureFramwork.PaymentHistory(array.getJSONObject(i));
        ...
        protected void onPostExecute(Boolean result) {
            ...
            if (result) {
                if (paymentHistory != null) {
                    ...
                    adapter = new PaymentHistoryDetailsAdapter(context, paymentHistory);
                ...
                if (error.length() > 0) {
                    ...
                    //SchoolAppUtils.showDialog(context, context.getTitle().toString(), error);
                } else {
                    Log.d("error4", "error");
                    SchoolAppUtils.showDialog(context, "Fees", getResources().getString(R.string.error));
                }
    ...
    private class GetCommission extends AsyncTask<List<NameValuePair>, Void, Boolean> {
        ...
        protected Boolean doInBackground(List<NameValuePair>... params) {
            ...
            JsonParser jParser = new JsonParser();
            JSONObject json = jParser.getJSONFromUrlfrist(namevaluepair, SchoolAppUtils.GetSharedParameter(context, Constants.COMMON_URL) + Urls.api_ais_getCommission);
            try {
                ...
                if (json != null) {
                    if (json.getString("result").equalsIgnoreCase("1")) {
                        ...
                        for (int i = 0; i < array.length(); i++) {
                            DataStructureFramwork.GatewayData pg_data = new DataStructureFramwork.GatewayData(array.getJSONObject(i));
        ...
        protected void onPostExecute(Boolean result) {
            ...
            if (result) {
                ...
                if (error.length() > 0) {
                    SchoolAppUtils.showDialog(context, "Registration", error);
                } else {
                    SchoolAppUtils.showDialog(context, "Registration", getResources().getString(R.string.error));
                }
    ...
    private class GetCCavenueInformation extends AsyncTask<List<NameValuePair>, Void, Boolean> {
        ProgressDialog dialog;
        String error = "";
        JSONObject data_obj;
        ...
        protected Boolean doInBackground(List<NameValuePair>... params) {
            ...
            JSONObject json = jParser.getJSONFromUrlfrist(namevaluepair, SchoolAppUtils.GetSharedParameter(context, Constants.COMMON_URL) + Urls.api_ais_getCcavenueDetails);
    ...
    private class GetRegistrationNumber extends AsyncTask<List<NameValuePair>, Void, Boolean> {
        ProgressDialog dialog;
        String error = "";
        String data_obj;
