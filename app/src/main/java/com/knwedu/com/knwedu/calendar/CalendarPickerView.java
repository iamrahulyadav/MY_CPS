...
import com.knwedu.com.knwedu.calendar.MonthCellDescriptor.RangeState;
...
public class CalendarPickerView extends ListView {
	...
	//private HashMap<Integer, ArrayList<Date>> hashmapOfEventDates;
	...
	public FluentInitializer init(Date minDate, Date maxDate, Locale locale) {
		...
		this.locale = locale;
		today = Calendar.getInstance(locale);
		SimpleDateFormat format = new SimpleDateFormat(getContext().getString(R.string.month_name_format), locale);
		Calendar cal = Calendar.getInstance();
		cal.add(Calendar.MONTH, -1);
		minCal = cal;//Calendar.getInstance(locale);
		maxCal = Calendar.getInstance(locale);
		monthCounter = Calendar.getInstance(locale);
		...
		while ((monthCounter.get(MONTH) <= maxMonth // Up to, including the
			...
			MonthDescriptor month = new MonthDescriptor(monthCounter.get(MONTH), monthCounter.get(YEAR), date,
	 ...
	 * {@link Locale#getDefault()}. If you wish the calendar to be
	...
	public class FluentInitializer {
		...
		public FluentInitializer withSelectedDates(Collection<Date> selectedDates) {
			...
			// if (selectionMode == SelectionMode.RANGE && selectedDates.size()
			// > 2) {
			// throw new IllegalArgumentException(
			// "RANGE mode only allows two selectedDates. You tried to pass " +
			// selectedDates.size());
			// }
		...
		public FluentInitializer withSelectedDates(HashMap<Integer, ArrayList<Date>> hashmap) {

			for (int i = 0; i < hashmap.size(); i++) {
				//hashmapOfEventDates = hashmap;
				highlightDates(hashmap.get(i + 1));
				System.out.println("HASHMAP" + hashmap.get(i + 1));
			}
			validateAndUpdate();
...
//					minCal.getTime(), maxCal.getTime(), date));
				...
				for (List<List<MonthCellDescriptor>> month : cells) {
					for (List<MonthCellDescriptor> week : month) {
						for (MonthCellDescriptor singleCell : week) {
							if (singleCell.getDate().after(start) && singleCell.getDate().before(end)
	...
	public void highlightDates(Collection<Date> dates) {
		for (Date date : dates) {
			...
			if (monthCellWithMonthIndex != null) {
				...
				selectedCals.add(newlyHighlightedCal);
					...
					for (List<List<MonthCellDescriptor>> month : cells) {
						for (List<MonthCellDescriptor> week : month) {
							for (MonthCellDescriptor singleCell : week) {
								if (singleCell.getDate().after(start) && singleCell.getDate().before(end)
										&& singleCell.isSelectable()) {
									if (selectedCells.get(0).isSelected() && selectedCells.get(1).isSelected()) {
	...
	List<List<MonthCellDescriptor>> getMonthCells(MonthDescriptor month, Calendar startCal) {
		...
		while ((cal.get(MONTH) < month.getMonth() + 1 || cal.get(YEAR) < month.getYear()) //
			...
			for (int c = 0; c < 7; c++) {
				...
				weekCells.add(new MonthCellDescriptor(date, isCurrentMonth, isSelectable, isSelected, isToday,
	...
	/**
	 * Set an adapter used to initialize {@link CalendarCellView} with custom
	 * layout.
	 * <p>
	 * Important: set this before you call {@link #init(Date, Date)} methods. If
	 * called afterwards, it will not be consistently applied.
	...
	private class DefaultOnInvalidDateSelectedListener implements OnInvalidDateSelectedListener {
		...
		public void onInvalidDateSelected(Date date) {
			String errMessage = getResources().getString(R.string.invalid_date, fullDateFormat.format(minCal.getTime()),
