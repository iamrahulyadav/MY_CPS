...
public class AnnouncementAdapter extends BaseAdapter {
    ...
    public AnnouncementAdapter(Context context, ArrayList<Announcement> assignments, int type, String title, String header) {
        this.context = context;
        this.assignments = assignments;
        check = new int[this.assignments.size()];
        this.type = type;
        ...
        holder.mnLayout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (type == 2) {
                    ...
                    intent.putExtra(Constants.ANNOUNCEMENT, assignments.get(position).object.toString());
                    if (header.trim().isEmpty())
                        intent.putExtra(Constants.PAGE_TITLE, "Announcement");
                    else
                        intent.putExtra(Constants.PAGE_TITLE, "" + header);
                    context.startActivity(intent);
                } else {
                    Intent intent = new Intent(context, ClassWorkActivity.class);
                    intent.putExtra(Constants.ANNOUNCEMENT, assignments.get(position).object.toString());
                    if (header.trim().isEmpty())
                        intent.putExtra(Constants.PAGE_TITLE, "Class Work");
                    else
                        intent.putExtra(Constants.PAGE_TITLE, "" + header);
                    context.startActivity(intent);
                }
