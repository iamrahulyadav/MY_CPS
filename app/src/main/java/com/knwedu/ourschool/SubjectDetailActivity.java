...
public class SubjectDetailActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_subject_detail_lesson_plan);
	...
	private void initialize() {
		((TextView) findViewById(R.id.header_text)).setText("Teacher Profile");
	...
	private void initializeData() {
		if (info != null) {
			...
			Log.d("Fname", info.fname);

			String[] tech_name = info.fname.split("\\,");
			...
			for (int i = 0; i < num_teacher_name; i++) {
				...
				SchoolAppUtils.setFont(SubjectDetailActivity.this, rowTextView);
				...
				rowTextView.setText(tech_name[i]);
				rowTextView.setBackgroundResource(R.drawable.blue_border_box);
				rowTextView.setTextColor(Color.parseColor("#37474F"));
				rowTextView.setPadding(16, 16, 16, 16);
				// add the textview to the linearlayout
			...
			if (info.image != null) {
				new LoadImageAsyncTask(SubjectDetailActivity.this, userImage,
						Urls.base_url + Urls.image_url_userpic, info.image,
						true).execute();
			}
