...
public class PaymentReminderActivity extends FragmentActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		reminderList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				Intent intent = new Intent(PaymentReminderActivity.this, AimsPaymentFeeDetailsActivity.class);
				...
				intent.putExtra("class_id", paymentReminder.get(position).class_id);
	...
	private class RequestPaymentRemindersAsyntask extends AsyncTask<List<NameValuePair>, Void, Boolean> {
		...
		protected Boolean doInBackground(List<NameValuePair>... params) {
			...
			// Log parameters:
			Log.d("url extension: ", Urls.api_get_payment_reminder_details);
			String parameters = "";
			...
			JSONObject json = jParser.getJSONFromUrlnew(nameValuePairs, Urls.api_get_payment_reminder_details);
			try {
				if (json != null) {
		...
		protected void onPostExecute(Boolean result) {
			...
			if (result) {
				...
				if (error.length() > 0) {
					SchoolAppUtils.showDialog(PaymentReminderActivity.this, page_title, error);
					reminderList.setAdapter(adapter);
				} else {
					SchoolAppUtils.showDialog(PaymentReminderActivity.this, page_title,
