...
public class ParentRequestFragment extends Fragment {
    ...
    private void initialize() {
        ...
        if (!SchoolAppUtils.GetSharedParameter(getActivity(), Constants.LEAVE_REQUEST).toString()
                .equalsIgnoreCase("0")) {
            request.add(SchoolAppUtils.GetSharedParameter(getActivity(),
                    Constants.LEAVE_REQUEST).toString());
        }
        if (!SchoolAppUtils.GetSharedParameter(getActivity(), Constants.REQUEST_FOR_ID_CARD).toString()
                .equalsIgnoreCase("0")) {
            request.add(SchoolAppUtils.GetSharedParameter(getActivity(), Constants.REQUEST_FOR_ID_CARD).toString());
        }
        ...
        if (!SchoolAppUtils.GetSharedParameter(getActivity(), Constants.REQUEST_FOR_BOOK)
                .toString().equalsIgnoreCase("0")) {
            request.add(SchoolAppUtils.GetSharedParameter(getActivity(),
                    Constants.REQUEST_FOR_BOOK).toString());
        }
        if (!SchoolAppUtils.GetSharedParameter(getActivity(), Constants.SPECIAL_REQUEST)
                .toString().equalsIgnoreCase("0")) {
            request.add(SchoolAppUtils.GetSharedParameter(getActivity(),
        ...
        listRequest.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> arg0, View arg1,
                ...
                Intent i = null;
                if (position == 0) {
                    i = new Intent(getActivity(), ParentChildLeaveActivity.class);
                    i.putExtra("request_id", "1");
                    i.putExtra(Constants.PAGE_TITLE, SchoolAppUtils.GetSharedParameter(getActivity(),
                            Constants.LEAVE_REQUEST).toString());
                } else if (position == 1) {
                    i = new Intent(getActivity(), ParentChildRequestActivity.class);
                    ...
                    i.putExtra(Constants.PAGE_TITLE,
        ...
        showRequestStatus.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                Intent intent = new Intent(getActivity(), ParentRequestStatusListActivity.class);
