...
public class ParentEditProfileActivity extends FragmentActivity {
	...
	String fnameS = null, lnameS = null, phoneS = null, addressS = null
			, emailS = null, religionS = null, bloodGroups = null;
	@Override
	...
	private void initialize() {
		...
		try {
			data = new JSONObject(SchoolAppUtils.GetSharedParameter(ParentEditProfileActivity.this, Constants.PUSERINFO));
		...
		String temp = getIntent().getExtras().getString(Constants.PARENT_INFO);
		if (temp != null) {
	...
	private void initializeData()
	{
		if(userInfo != null)
		{
			...
			String imageUrl = getIntent().getExtras().getString(
					Constants.IMAGE_URL);
			new LoadImageAsyncTask(ParentEditProfileActivity.this, img, Urls.image_url_userpic, imageUrl, true).execute();
		...
		submite.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				{
					Toast.makeText(ParentEditProfileActivity.this, "Enter First Name", Toast.LENGTH_SHORT).show();
					return;
				}
				if(lastName.getText().toString().trim().length() <= 0)
				{
					Toast.makeText(ParentEditProfileActivity.this, "Enter Last Name", Toast.LENGTH_SHORT).show();
					return;
				}
				if(phone.getText().toString().trim().length() <= 0)
				{
					Toast.makeText(ParentEditProfileActivity.this, "Enter Phone Number", Toast.LENGTH_SHORT).show();
					return;
				}
				if(address.getText().toString().trim().length() <= 0)
				{
					Toast.makeText(ParentEditProfileActivity.this, "Enter Address", Toast.LENGTH_SHORT).show();
					return;
				}
				if(religion.getText().toString().trim().length() <= 0)
				{
					Toast.makeText(ParentEditProfileActivity.this, "Enter Religion", Toast.LENGTH_SHORT).show();
					return;
				}

				if(bloodGroup.getText().toString().trim().length() <= 0)
				{
					Toast.makeText(ParentEditProfileActivity.this, "Enter Blood Group", Toast.LENGTH_SHORT).show();
					return;
				}
				if(email.getText().toString().trim().length() <= 0)
				{
					Toast.makeText(ParentEditProfileActivity.this, "Enter Email Address", Toast.LENGTH_SHORT).show();
					return;
				}
				if(pincode.getText().toString().trim().length() <= 0)
				{
					Toast.makeText(ParentEditProfileActivity.this, "Enter Pin Code", Toast.LENGTH_SHORT).show();
					return;
				}

	/*			try {fnameS = URLEncoder.encode(firstName.getText().toString(), "utf-8");} catch (UnsupportedEncodingException e) {}
				try {lnameS = URLEncoder.encode(lastName.getText().toString(), "utf-8");} catch (UnsupportedEncodingException e) {}
				try {phoneS = URLEncoder.encode(phone.getText().toString(), "utf-8");} catch (UnsupportedEncodingException e) {}
				try {emailS = URLEncoder.encode(email.getText().toString(), "utf-8");} catch (UnsupportedEncodingException e) {}
				try {addressS = URLEncoder.encode(address.getText().toString(), "utf-8");} catch (UnsupportedEncodingException e) {}
				try {religionS = URLEncoder.encode(religion.getText().toString(), "utf-8");} catch (UnsupportedEncodingException e) {}
				try {bloodGroups = URLEncoder.encode(bloodGroup.getText().toString(), "utf-8");} catch (UnsupportedEncodingException e) {}

*/				//64.150.189.99/mobility2/webservices/parents.php?mode=edit_profile&
				//email=john@parent.com&password=smonte&fname=&lname=&religion=&address=&phone=&blood=

		/*		String url = URLs.EditParentProfile+ 
						"email=" + emailSSchoolAppUtils.GetSharedParameter(ParentEditProfileActivity.this, Constants.PUSERNAME) + 
						"&password=" +SchoolAppUtils.GetSharedParameter(ParentEditProfileActivity.this, Constants.PPASSWORD) +
						"&fname=" + fnameS+
						"&lname=" + lnameS+
						"&religion=" +religionS +
						"&phone=" + phoneS +
...
*/
				 List<NameValuePair> nameValuePairs = new ArrayList<NameValuePair>(11);
			        nameValuePairs.add(new BasicNameValuePair("user_id", SchoolAppUtils.GetSharedParameter(ParentEditProfileActivity.this, Constants.USERID)));
			        nameValuePairs.add(new BasicNameValuePair("user_type_id", SchoolAppUtils.GetSharedParameter(ParentEditProfileActivity.this, Constants.USERTYPEID)));
			        nameValuePairs.add(new BasicNameValuePair("organization_id", SchoolAppUtils.GetSharedParameter(ParentEditProfileActivity.this, Constants.UORGANIZATIONID)));
	...
	private class UploadAsyntask extends AsyncTask<List<NameValuePair>, Void, Boolean>
	{
		...
		protected void onPreExecute() {
			super.onPreExecute();
			dialog = new ProgressDialog(ParentEditProfileActivity.this);
		...
		protected void onPostExecute(Boolean result) {
			...
			if(result)
			{
				dialoggg = new AlertDialog.Builder(ParentEditProfileActivity.this);
				...
				dialoggg.setNeutralButton(R.string.ok, new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface arg0, int arg1) {
						...
						userInfo.blood_group = bloodGroup.getText().toString();
						userInfo.email = email.getText().toString();
						
						userInfo.updateObject();
						
						SchoolAppUtils.SetSharedParameter(ParentEditProfileActivity.this, Constants.PUSERNAME, email.toString());
						SchoolAppUtils.SetSharedParameter(ParentEditProfileActivity.this, Constants.PUSERINFO, userInfo.toString());
						SchoolAppUtils.SetSharedBoolParameter(ParentEditProfileActivity.this, "update", true);
						*/if(selectedImagePath != null)
						{
...
//							startService(intent);
						...
						ParentEditProfileActivity.this.finish();
					}
				});
				dialoggg.show();
			}
			else
			{
				if(error != null)
				{
					if(error.length() > 0)
					{
						SchoolAppUtils.showDialog(ParentEditProfileActivity.this,getTitle().toString(), error);
					}
					else
					{
						SchoolAppUtils.showDialog(ParentEditProfileActivity.this,getTitle().toString(), 
	...
	private void showDialogBoxForImages()
	{
		...
		AlertDialog.Builder alertbox = new AlertDialog.Builder(
				ParentEditProfileActivity.this);
		...
		if (resultCode == RESULT_OK) {
			if(requestCode == SELECT_PICTURE || requestCode == SELECT_CAMERA)
			{
				...
				catch(Exception e)
				{
					...
					Toast.makeText(ParentEditProfileActivity.this, R.string.please_try_again, Toast.LENGTH_SHORT).show();
				}
				if(!checkError)
				{
...
opts.inSampleSize = originalSize;
	...
	{
		innerImageLoader = new InnerImageLoader(ParentEditProfileActivity.this);
		innerImageLoader.clearCache();
